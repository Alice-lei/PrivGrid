...
int paillier_ell(mpz_t result, mpz_t input, mpz_t ninv, mp_bitcnt_t len) {
	...
	mpz_init(mask);
	//gmp_printf ("paillier_ell %Mu\n", len);
	//gmp_printf ("paillier_ell %d\n", len);


	mpz_sub_ui(result, input, 1);
	mpz_mul(result, result, ninv);
	//debug_msg("sucessful mul0\n");
	mpz_setbit(mask, len);
	//debug_msg("sucessful mul1\n");
	mpz_sub_ui(mask, mask, 1);
	//debug_msg("sucessful mul2\n");
	mpz_and(result, result, mask);
	//debug_msg("sucessful mul3\n");
...
int paillier_decrypt(mpz_t plaintext, mpz_t ciphertext, paillier_private_key *priv) {
	...
	paillier_ell(plaintext, plaintext, priv->ninv, mlen);
    //printf("ell is ok \n");
