...
//数据检查
void Lstm::dataCk(int dataSetindex)
{
    cout << "Data checking" << flush;
    ifstream infile;
    string line;
    string fileName;
    fileName = (role == SERVER) ? "SERVER" : "CLIENT";
    ...
    while (getline(infile, line))
        ++this->counts;
    infile.close();
    cout << "\rThere are a total of " << this->counts << " data." << endl;
}
...
void Lstm::train()
{
    ...
    for (int dataSetIndex = dataSetStartIndex; dataSetIndex < dataSetNum; dataSetIndex++)
    {
        ...
        string fileName = "./output/ValueTrain_DataSet_" + to_string(dataSetIndex + 1) + "_" + roleStr + "_" + to_string(dataSetIndex + 1) + ".csv";
        string fileNameTime = "./output/TimeTrain_DataSet_" + to_string(dataSetIndex + 1) + "_" + roleStr + "_" + to_string(dataSetIndex + 1) + ".csv";
        string fileNameTest = "./output/ValueTest_DataSet_" + to_string(dataSetIndex + 1) + "_" + roleStr + "_" + to_string(dataSetIndex + 1) + ".csv";
        string fileNameTimeTest = "./output/TimeTest_DataSet_" + to_string(dataSetIndex + 1) + "_" + roleStr + "_" + to_string(dataSetIndex + 1) + ".csv";
        FILE *fptr = fopen(fileName.c_str(), (const char *)opt);
        FILE *fptrT = fopen(fileNameTime.c_str(), (const char *)opt);
        ...
        for (int userIndex = userStart; userIndex < userCounts; userIndex++) //userCounts
        {
            ...
            for (int trainingIndex = 0; trainingIndex < trainingRounds; trainingIndex++)
            {
                ...
                cout << "Data Set(" << dataSetIndex + 1 << "|5)"
                     << " | User(" << userIndex + 1 << "|" << userCounts << ")"
                     << " | Training(" << trainingIndex + 1 << "|" << trainingRounds << ") OK" << endl;
                /*string roundSaveFileName = "./input/matrix/round/";
            roundSaveFileName += (role == SERVER) ? "SERVER" : "CLIENT";
            roundSaveFileName += "/";
            roundSaveFileName += (role == SERVER) ? "SERVER" : "CLIENT";
            roundSaveFileName += "_round_" + to_string(i + 1) + "_save.dat";
            this->saveMatrix(roundSaveFileName);*/
            }
            ...
            // this->saveMatrix(saveFileName);
            this->test(userIndex, fptrTesting, fptrTimeTesting);
        }
        fclose(fptr);
        fclose(fptrT);
        fclose(fptrTesting);
        fclose(fptrTimeTesting);
    }
}
...
void Lstm::test(int userIndex, FILE *fptr, FILE *fptrT)
{
    ...
    {
        cout << "\nTesting No." << testingIndex + 1 << " Begin" << endl;
        this->tripesInit(tripesFLAG, prefix[1]);
        this->dataFill(testingIndex, TEST);
        lastPoint = time(nullptr);
        this->forwardNetwork();
        int roundTimeForword = showTime("TimeTestForward");
        int netTime = showNetTime("TimeTestNet");
        fprintf(fptrT, "%d,%d\n",
                roundTimeForword,
                netTime);
        fflush(fptrT);
